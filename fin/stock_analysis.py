...
# run command only in notebook environment
try:
    # get_ipython can be only run in notebooks, but not in scripts
    get_ipython
...
def persist_figure(qf_fig: go.Figure, ticker: yf.Ticker):
    ...
    qf_fig.write_html(f"""{ticker}_quantFig_{today.strftime("%Y_%m_%d")}.html""")
...
def extract_ticker_symbols():
    ...
    tickers_list = list(pd.concat([nasdaq_listed, other_listed], axis=0).Symbol.unique())
